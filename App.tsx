    ...
    switch (state.currentStep) {
      case 1: return (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelClass}>Nivel Académico</label><input value={data.nivelAcademico} onChange={e => updateField('nivelAcademico', e.target.value)} className={inputClass + " uppercase"} /></div>
            <div><label className={labelClass}>Ocupación</label><input value={data.ocupacion} onChange={e => updateField('ocupacion', e.target.value)} className={inputClass + " uppercase"} /></div>
          </div>
          <div className="grid grid-cols-3 gap-3">
      ...
      case 3: return (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelClass}>Folio IA</label><input value={data.folio} onChange={e => updateField('folio', e.target.value)} className={inputClass + " border-black text-indigo-600"} /></div>
      ...
      case 4: return (
        <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
          <div className="bg-black text-[#D4AF37] p-4 rounded-2xl flex justify-between items-center mb-4 sticky top-0 z-10">
            <span className="text-xs font-black uppercase">Deudas Extraídas IA ({data.bancos.length})</span>
            <span className="text-[10px] font-bold bg-[#D4AF37] text-black px-3 py-1 rounded-full">{Math.ceil(data.bancos.length / 6)} BLOQUES</span>
          </div>
          {data.bancos.map((b, i) => (
        ...
        return (
          <div className="space-y-6">
            <div className="bg-black text-[#D4AF37] p-6 rounded-[32px] border-2 border-[#D4AF37]/30 mb-8">
              <h3 className="text-lg font-black uppercase tracking-tighter italic">Centro de Inyección UnoRep Forms IA</h3>
              <p className="text-[11px] text-white/60 font-medium uppercase tracking-widest mt-1">
                {data.bancos.length} deudas totales • {debtBlocks} bloques generados para llenado completo.
              </p>
            </div>

            <div className="space-y-8">
              <div className="space-y-3">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Módulos de Carátula (Inyección por Bloques)</h4>
                {[...Array(debtBlocks)].map((_, i) => (
                  <button key={`caratula-${i}`} onClick={() => window.open(getFilledUrl('https://docs.google.com/forms/d/e/1FAIpQLSfonP1hkAxUxR1EzN0SV2Apl4PJ8jvQ1phELIT6dUwwWk9kJw/viewform', i), '_blank')}
                    ...
                    <div className="flex flex-col text-left">
                      <span className="font-black text-sm uppercase tracking-tight">Carátula Contrato • Bloque {i+1}</span>
                      <span className="text-[10px] opacity-60 font-bold uppercase">Deudas {i*6+1} a {Math.min((i+1)*6, data.bancos.length)}</span>
                    </div>
              ...
              <div className="space-y-3">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Módulos de Carta Poder (Inyección por Bloques)</h4>
                {[...Array(debtBlocks)].map((_, i) => (
                  <button key={`poder-${i}`} onClick={() => window.open(getFilledUrl('https://docs.google.com/forms/d/e/1FAIpQLSck3u0JAoWosvZ3WXYqUPbttJVuXbule0xD1V3ejgAB-gM-9w/viewform', i, true), '_blank')}
                    ...
                    <div className="flex flex-col text-left">
                      ...
                      <span className="text-[10px] text-slate-400 font-bold uppercase group-hover:text-slate-600">Deudas {i*6+1} a {Math.min((i+1)*6, data.bancos.length)}</span>
                    </div>
              ...
              <div className="space-y-3">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Formularios Complementarios (Titular)</h4>
        ...
        <div className="max-w-4xl mx-auto py-12 animate-in fade-in zoom-in duration-700">
          <div className="text-center mb-16">
            ...
            <h2 className="text-7xl font-black text-black tracking-tighter mb-4 leading-none uppercase">
              UnoRep <span className="text-[#D4AF37] italic">Forms IA</span>
            </h2>
            <p className="text-slate-400 text-xl font-medium max-w-xl mx-auto italic leading-tight">Gestión inteligente de deudas y automatización avanzada UNOREP.</p>
          </div>
          
          <div className="max-w-xl mx-auto mb-10">
            <FileUpload 
              label="Bóveda de Expedientes" 
      ...
      {state.status === FormStatus.EXTRACTING && (
        <div className="flex-1 flex flex-col items-center justify-center py-20 text-center animate-pulse">
          <div className="relative mb-12">
            ...
            <div className="absolute inset-0 flex items-center justify-center font-black text-[#D4AF37] text-xl">IA</div>
          </div>
          <h2 className="text-4xl font-black text-black tracking-tighter uppercase mb-2 italic">Extrayendo Datos...</h2>
        ...
        <div className="flex flex-col lg:flex-row gap-10 py-6">
          <aside className="lg:w-80 space-y-2 animate-in slide-in-from-left duration-500">
            <div className="mb-10 p-7 bg-black rounded-[40px] border-2 border-[#D4AF37]/40 shadow-2xl">
              <span className="text-[9px] font-black text-[#D4AF37] uppercase tracking-[0.3em] block mb-2 opacity-50">Titular del Expediente</span>
              <p className="font-black text-white text-xl tracking-tight uppercase leading-tight truncate">{state.data.nombreCompleto || 'SIN NOMBRE'}</p>
              <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-center">
                <span className="text-[10px] font-black text-[#D4AF37]">FOLIO: {state.data.folio || '---'}</span>
                <span className="bg-[#D4AF37] text-black text-[8px] font-black px-3 py-1 rounded-full uppercase shadow-lg">LISTO</span>
              </div>
            </div>
          ...
          <div className="flex-1 bg-white rounded-[60px] p-10 lg:p-16 shadow-[0_40px_100px_rgba(0,0,0,0.04)] border border-slate-50 relative flex flex-col animate-in slide-in-from-right duration-500 min-h-[700px]">
            <header className="mb-12 flex items-center justify-between border-b-8 border-black pb-10">
              ...
              <div className="flex flex-col items-end">
                <span className="text-xs font-black bg-black text-[#D4AF37] px-6 py-2 rounded-full tracking-widest uppercase">Módulo {state.currentStep} / 6</span>
              </div>
            </header>
            ...
            <footer className="flex justify-between mt-16 pt-10 border-t border-slate-100">
              ...
              {state.currentStep < 6 ? (
                ...
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-black text-[#D4AF37] uppercase tracking-[0.5em] mb-2 animate-pulse">Llenado IA Disponible</span>
